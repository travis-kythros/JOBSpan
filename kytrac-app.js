// KYTRAC Application JavaScript
// Generated from kytrac.html
...
function conLoadFirebase() {
  ...
  // If Firebase already loaded (e.g. cached from previous session), just init
  if (typeof firebase !== 'undefined' && firebase.apps !== undefined) {
    conInitFirebase();
    return;
  }
  // Prevent adding duplicate script tags
  if (document.querySelector('script[src*="firebase-app-compat"]')) {
    const wait = setInterval(function() {
      if (typeof firebase !== 'undefined') { clearInterval(wait); conInitFirebase(); }
    }, 100);
    return;
  }
...
// ════════════════════════════════════════════════════
// ── Old flat estimate code removed — replaced by hierarchical estimate engine below ──
// renderCOList and conLoadCOs are defined in the CO section below
...
function conLoadCOs(jobId) {
  ...
  conDb.collection('jobs').doc(jobId).collection('changeOrders')
    ...
    .then(snap => {
      ...
      renderCOList();
    }).catch(e => console.error('CO load error:', e));
}

function renderCOList() {
  const el = document.getElementById('detailChangeorders');
  if (!el) return;
  const job = conJobs.find(j => j.id === conCurrentJobId);
  const totalCOs = conCOs.reduce((s, co) => s + (co.status === 'Approved' ? (parseFloat(co.amount)||0) : 0), 0);
  ...
  el.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-wrap:wrap;gap:8px">' +
    '<div><div style="font-weight:700">Change Orders</div>' +
    (totalCOs ? '<div class="small muted">Approved COs: <strong style="color:#1dbb87">$'+totalCOs.toLocaleString()+'</strong></div>' : '') +
    '</div>' +
    '<button class="btn-amber" onclick="openAddCOModal()" style="padding:6px 14px;font-size:.82rem">+ Add CO</button></div>' +
    (conCOs.length === 0
      ? '<div class="small muted" style="text-align:center;padding:20px">No change orders yet.</div>'
      : conCOs.map(co => {
          const statusClass = {Pending:'co-pending',Approved:'co-approved',Rejected:'co-rejected',Void:'co-void'}[co.status]||'co-pending';
          return '<div class="co-card"><div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;flex-wrap:wrap;gap:8px">' +
            '<div><div style="font-weight:700">'+(co.title||'Change Order')+'</div>' +
            '<div class="small muted" style="margin-top:2px">'+(co.date||'')+'</div></div>' +
            '<div style="display:flex;gap:8px;align-items:center">' +
            '<span style="font-size:1rem;font-weight:900;color:var(--amber)">$'+(parseFloat(co.amount)||0).toLocaleString()+'</span>' +
            '<span class="co-badge '+statusClass+'">'+co.status+'</span>' +
            '<button onclick="openEditCO(\"'+co.id+'\")" class="btn" style="padding:2px 8px;font-size:.75rem">Edit</button>' +
            '<button onclick="deleteCO(\"'+co.id+'\")" class="btn btn-danger" style="padding:2px 8px;font-size:.75rem">✕</button>' +
            '</div></div>' +
            (co.description ? '<div class="small muted">'+esc(co.description)+'</div>' : '') +
            '</div>';
        }).join(''));
}
...
// Bid loading is handled directly in switchDetailTab below
...
function openAddGroupModal(editId) {
  ...
  const common = ['Exterior','Interior','Kitchen','Master Bath','Bathroom','Bedroom','Living Room','Basement','Laundry Room','Dining Room','Deck / Porch','Roof','Siding','Windows & Doors','Garage','Hallway','Demo / Trash Out','Miscellaneous'];
  ...
  if (qp) {
    ...
    suggestions.forEach(function(n) {
      var btn = document.createElement('button'); btn.className='btn'; btn.textContent=n;
...
function openAddSubgroupModal(groupId, editSubId) {
  ...
  if (!targetGroupId) {
    ...
    if (estGroups.length === 1) {
      ...
      // Show group picker inline
      _pendingSubgroupForGroup = null;
      _pendingSubgroupEditId = editSubId || null;
      openGroupPickerForSubgroup();
      return;
    }
  }
  ...
  // Quick picks based on group name
  const groupName = (group?.name || '').toLowerCase();
  let suggestions = [];
  if (groupName.includes('bath')) suggestions = ['Shower','Tub','Toilet','Vanity','Tile Floor','Mirror','Fixtures','Demo'];
  else if (groupName.includes('kitchen')) suggestions = ['Cabinets','Countertops','Backsplash','Appliances','Plumbing','Electrical','Flooring','Demo'];
  ...
  else if (groupName.includes('basement')) suggestions = ['Flooring','Framing','Drywall','Electrical','Plumbing','Painting','Egress'];
  ...
  setTimeout(() => document.getElementById('subgroupNameInput')?.focus(), 100);
}

let _pendingSubgroupForGroup = null;
let _pendingSubgroupEditId = null;

function openGroupPickerForSubgroup() {
  const names = estGroups.map(g => g.name).join(', ');
  const choice = estGroups.map((g,i) => (i+1)+'. '+g.name).join(' | ');
  const num = prompt('Which group? '+choice+' -- Enter number:');
  const idx = parseInt(num) - 1;
  if (isNaN(idx) || idx < 0 || idx >= estGroups.length) return;
  openAddSubgroupModal(estGroups[idx].id, _pendingSubgroupEditId);
}
    ...
    conDb.collection('jobs').doc(conCurrentJobId).collection('estimateGroups')
      .doc(groupId).collection('subgroups').add({
        ...
        if (group) {
          ...
          group.subgroups.push({ id: ref.id, name, order, items: [] });
        }
        renderEstimateTree();
      }).catch(e => alert('Error: ' + e.message));
  }
...
window.openGroupPickerForSubgroup = openGroupPickerForSubgroup;
  ...
  for (const item of itemsToAdd) {
    ...
    if (item.labor) {
      const labData = {
        ...
        unit: 'hr',
        costType: 'Labor',
        unitCost: item.labor.unitCost || 100,
        markup: 15,
        unitPrice: item.labor.unitPrice || 115,
        notes: '',
        order: order++,
        createdAt: firebase.firestore.FieldValue.serverTimestamp()
      };
